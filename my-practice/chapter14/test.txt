...
struct Sales_data
{
    ...
    friend ostream &operator<<(ostream &output, const Sales_data &transaction);
    friend istream &operator>>(istream &input, Sales_data &transaction);
    friend Sales_data &operator+(const Sales_data &first_transaction,
                                 const Sales_data &second_transaction);
    friend Sales_data &operator+=(Sales_data &first_transaction,
                                  const Sales_data &second_transaction);
...
ostream &operator<<(ostream &output, const Sales_data &transaction)
{
    output << transaction.isbn() << " "
           << transaction.units_sold << " "
           << transaction.revenue;
...
istream &operator>>(istream &input, Sales_data &transaction)
{
    double price = 0.0;
    input >> transaction.bookNo >> transaction.units_sold >> price;
    transaction.revenue = transaction.units_sold * price;
...
Sales_data &operator+(const Sales_data &first_transaction,
                      const Sales_data &second_transaction)
{
    Sales_data sum = first_transaction;
    sum.combine(second_transaction);
...
Sales_data &operator+=(Sales_data &first_transaction,
                       const Sales_data &second_transaction)
{
    first_transaction.combine(second_transaction);
